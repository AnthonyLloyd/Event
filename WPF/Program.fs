...
let main _ =
    ...
    let kidStore =
        ...
        |> List.iter (fun n -> Store.create oldUser [Kid.Name n;randomAge();randomBehaviour()] store |> ignore)
        store
    ...
    let toyStore =
        ...
        |> List.iter (fun n -> Store.create oldUser [Toy.Name n;randomAgeRange();randomWorkRequired()] store |> ignore)
        store
    ...
    let elfStore =
        ...
        |> List.iter (fun n -> Store.create oldUser [Elf.Name n;randomWorkRate()] store |> ignore)
        store
